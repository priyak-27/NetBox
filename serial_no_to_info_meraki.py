...
# Check if 'meraki_devices.csv' exists in the current directory and delete it if it does
csv_file_path = 'meraki_devices.csv'
...
cisco_meraki_manufacturer = next((m for m in manufacturers if m.name.lower() == 'cisco meraki'), None)

if not cisco_meraki_manufacturer:
    print("Cisco meraki manufacturer not found in NetBox.")
    exit()

# Fetch all devices with the manufacturer "Cisco"
devices = response.dcim.devices.filter(manufacturer_id=cisco_meraki_manufacturer.id)
...
print("Serial number for all devices has been added to meraki_devices.csv")
